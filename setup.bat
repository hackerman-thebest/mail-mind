...
python -c "import sys; sys.path.insert(0, 'src'); from mailmind.utils.system_diagnostics import check_system_resources, recommend_model, format_resource_report; import json; r = check_system_resources(); print(format_resource_report(r)); model, reasoning, perf = recommend_model(r); print(''); print('RECOMMENDED MODEL:'); print('  ' + model); print('  ' + reasoning); print('  Expected speed: ' + perf['tokens_per_second']); print('  Download size: ' + str(perf['size_gb']) + 'GB'); model_size = 'small' if '1b' in model else ('medium' if '3b' in model else 'large'); open('temp_recommendation.json', 'w').write(json.dumps({'model': model, 'size': model_size, 'ram': r['ram']['available_gb']}))"
